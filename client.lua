...
RegisterNetEvent("ds_cuff_item:checkCuff")
AddEventHandler("ds_cuff_item:checkCuff", function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if distance~=-1 and distance<=3.0 then
        ESX.TriggerServerCallback("ds_cuff_item:isCuffed",function(cuffed)
            if not cuffed then
                TaskPlayAnim(PlayerPedId(), "mp_arrest_paired", "cop_p2_back_right", 8.0, -8, 4000, 48, 0, 0, 0, 0) 
                TriggerServerEvent("ds_cuff_item:handcuff",GetPlayerServerId(player),true)
            else
                TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'a_uncuff', 8.0, -8, 5000, 49, 0, 0, 0, 0)
                TriggerServerEvent("ds_cuff_item:handcuff",GetPlayerServerId(player),false)
            end
            TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 4.0, 'busted', 1.0)
        end,GetPlayerServerId(player))
    else
        ESX.ShowNotification("~r~Šalia nėra asmenų!")
    end
end)

RegisterNetEvent("ds_cuff_item:uncuff")
AddEventHandler("ds_cuff_item:uncuff",function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if distance~=-1 and distance<=3.0 then
        TriggerServerEvent("ds_cuff_item:uncuff",GetPlayerServerId(player))
    else
        ESX.ShowNotification("~r~Šalia nėra asmenų")
    end
end)

RegisterNetEvent('ds_cuff_item:forceUncuff')
AddEventHandler('ds_cuff_item:forceUncuff',function()
    IsHandcuffed = false
...
RegisterNetEvent("ds_cuff_item:handcuff")
AddEventHandler("ds_cuff_item:handcuff",function()
    local playerPed = GetPlayerPed(-1)
